...
@section('content')
    <article class="blogs">
        <h1 class="t_nav">
            ...
            <a href="{{url('lists/'.$cate->cate_id)}}" class="n2">{{$cate->cate_name}}</a>
        </h1>
...
{{--                <img src="{{url('images/ad.png')}}">--}}
        ...
        <aside class="right">
            <div class="rnav">
                <ul>
                    @foreach($cate_two as $k=>$v)
                        <li class="rnav{{$k+1}}"><a href="{{url('lists/'.$v->cate_id)}}" target="_blank">{{$v->cate_name}}</a></li>
                    @endforeach
                </ul>
            </div>
            <div class="news">

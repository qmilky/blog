...
@section('content')
    <article class="blogs">
        ...
        <h1 class="t_nav">
            ...
            <a href="#" class="n2">{{$cate['cate_name']}}</a>
{{--            <a href="{{url('lists/'.$cate['id'])}}" class="n2">{{$cate['cate_name']}}</a>--}}
        ...
        <div class="newblog left">
            ...
            @foreach($arts as $k=>$v)
                <h2>{{$v->art_title}}</h2>
                <p class="dateview"><span>发布时间：{{$v->art_time}}</span><span>作者：{{$v->art_editor}}</span><span>分类：[<a href="{{url('lists/'.$cate['id'])}}">{{$cate['cate_name']}}</a>]</span></p>
...
{{--                    {!! $arts->appends(['cate'=>$cate,'id'=>$cate['id']])->render() !!}--}}
                    {{$arts->appends(['cate'=>$cate,'id'=>$cate['id']])->links()}}
                </ul>

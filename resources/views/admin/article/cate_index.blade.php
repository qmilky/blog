...
@section('title')
    <title>{{config('cate_article_index')}}</title>
@endsection
@section('body')
    <!--面包屑导航 开始-->
...
//                $.get("请求服务器的路径","携带的参数", 获取执行成功后的额返回数据);
                //admin/user/1
                $.post("{{url('admin/article/cates/del')}}/"+id,{"_token":"{{csrf_token()}}"},function(data){
                {{--$.post("{{url('admin/article/cates/del')}}/"+id,{"_method":"delete","_token":"{{csrf_token()}}"},function(data){--}}
                    //alert(data);
